...
- Version unique dans `package.json` (0.1.0) : Vite l'expose en `__APP_VERSION__` (`vite.config.ts`, déclarée dans `src/env.d.ts`), Gradle en tire `versionName` et `versionCode` (majeur×10000 + mineur×100 + correctif, 100 pour 0.1.0). À augmenter avant chaque APK diffusé.
