...
| `tracker.sections` | `hooks/useOpenSections.ts` | `Record<moduleId, Record<section, boolean>>` ; identifiants `running` et `sailing-onglets` (barre unique de la voile, point 57) ; `sailing` et `sailing-carte`, d'avant, ne sont plus lus |
...
- Mise en page des analyses : deux passes faites et validées (§10, points 56 et 57). Défaut connu, proposé et pas traité : valeur haute de l'axe du graphe « Évolution du vent » tronquée (« 51536° »).
