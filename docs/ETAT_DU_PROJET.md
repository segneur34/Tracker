...
- Hors plan, le 25 septembre 2026 : lot I de la liste de bugs du 24/09 (point 47, APK 0.2.3) — quatre bugs d'interface téléphone (saisie du profil coureur, bascule des panneaux voile, disposition de la carte, aperçu carte de la bibliothèque). Lot II (point 48, APK 0.2.5) : renommage d'une session, depuis la liste ou l'en-tête de l'analyse ; mise en page téléphone du lot I portée en course. Restent la pause d'enregistrement et le poids du GPX (lot III), sous Opus.
- Phase 2 : interface mobile. Amorcée pour les modules voile et course (points 47 et 48) : carte pleine largeur en haut sous 768 px, feuille qui la chevauche avec quatre chiffres clés, plein écran au tap, panneaux repliables par leur titre (`analysisMobile.css`). Restent : disposition empilée pour le reste de l'interface, taille des onglets et du texte à revoir plus largement (retour de l'utilisateur, point 47) ; toucher au lieu du survol ; graphes à largeur fixe (500 et 350 px) et poignée `resize` de 20 px à revoir ; `preferCanvas` pour la carte, qui porte une `Polyline` par segment (10 800 pour 3 h à 1 Hz) : les regrouper par couleur toucherait à la décision « pas de paliers », à redemander ; `accept=".gpx"`, qui grise parfois les GPX sous Android.
