...
- Graphes course « superposés » à deux axes, voulus malgré la difficulté de lecture. Courbe d'altitude et aire dessous colorées par la raideur de la pente, montée ou descente confondues, sur la palette de la trace, de 0 à 25 % par défaut, bornes réglables par activité dans Réglages.
- Tout bloc est redimensionnable (`ResizablePanel`), taille mémorisée.
- Voile : manœuvres en petit tableau, détails dépliables. Carte à 60 % de large ; à sa droite, une seule barre d'onglets : tops, manœuvres, graphiques, VMG, vent, matos et conditions, réglages (dans cet ordre, fermés par défaut, côte à côte si la place le permet). La fiche du haut, toujours visible, porte les chiffres globaux (vitesse moyenne et moyenne active comprises) et le vent. Ne rien déplacer ni dupliquer sans redemander.
...
- Réglages d'affichage (unités de vitesse et de distance, taille du texte) choisis dans Réglages seulement, par activité (donc par support : bateau et wing peuvent différer), actifs partout ; distance en km par défaut, milles nautiques au choix. La pause automatique se règle aussi par activité.
